...
const TrackFiles = () => {
    ...
    return (
        <Box sx={{ padding: '20px 60px' }}>
            ...
            <Typography sx={{ paddingLeft: '10px', fontWeight: 'bold', marginBottom: '10px' }}>{data.id}</Typography>

            <Stepper activeStep={-1} orientation="vertical" sx={{ maxHeight: '70vh', overflow: 'scroll' }}>
                {steps.map((step, index) => (
                    <Step key={index}>
                        <StepLabel optional={<Typography variant="caption">on {step.assignedon}</Typography>}>
